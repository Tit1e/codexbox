 ...
 * [OUTPUT]: 验证项目运行命令的顶栏状态、继承规则动作和删除配置
...
function setup({ rule: initialRule, cwd = '/repo/src', states = [] } = {}) {
  ...
  const calls = { api: [], apiPost: [], menus: [], started: [], runningRoots: [] };
  const term = {
    available: () => true,
...
test('精确规则可以移除，移除后回到仅设置状态', async () => {
  ...
  try {
    ...
    await settle();
    document.querySelector('.project-run-settings').click();
    const remove = calls.menus.at(-1).find((item) => item.label === '移除运行命令');
    assert.equal(remove.danger, true);
    await remove.fn();
    await settle();
    assert.deepEqual(calls.apiPost, [['/api/run-rule/delete', { path: '/repo' }]]);
    assert.ok(document.querySelector('.project-run-settings'));
    assert.equal(document.querySelector('.project-run-start'), null);
  } finally { dom.cleanup(); }
});
