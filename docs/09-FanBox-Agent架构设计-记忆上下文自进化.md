<!--
[INPUT]: 依赖 FanBox 当前终端架构、OpenClaw/Hermes 研究资料与 Agent 记忆相关论文
[OUTPUT]: 对外提供 FanBox Agent 记忆、上下文和自主进化的研究结论与候选路线
[POS]: docs 的前瞻架构文档，描述候选设计而非当前已交付功能
...
> 设计视角：克制、用完即走、做减法、警惕过度设计、用户价值压倒功能堆砌。叠加 FanBox 第一原则：**先服务花叔自己的工作流，不为外部用户牺牲他的体验**。
> 日期：2026-06-15
...
所以这份设计的核心动作不是「加」，而是「**接 + 减**」：接通已有零件，减掉别人为规模付出的重量。
...
| 本地 CLI 入口（不经中转） | Electron + node-pty 承载用户已安装的 coding agent | ✅ 已有 |
...
┌───────────────────────────── FanBox Agent OS ─────────────────────────────┐
│                                                                            │
│  入口层        FanBox 终端 ──► node-pty ── cwd=当前项目 ────────┐           │
│                                                                 ▼           │
│  大脑层                                  用户选择的本机 coding agent CLI     │
│                         （无中转，原生读取 cwd 的项目指令与上下文）          │
│                                                 │         │        │        │
│        ┌────────────────────────────────────────┘         │        │        │
│        ▼                          ▼                        ▼        ▼        │
...
2. **上下文**：有界输入 + 压缩前先 flush + 子 agent 卸载 + 文件外存 + altitude 审计。
3. **能力**：huashu-design（表达）/ nuwa（造 skill）/ darwin（进化 skill）作为 agent 的三个可调用引擎，skills 渐进披露。
4. **人格（Soul）**：复用各 CLI 原生的全局与项目级指令文件，不在 FanBox 重造一套。
...
上下文窗口是有限资源，终端里的 coding agent 又各自掌握真实会话，FanBox 应先守住边界，再考虑增强：

1. **压缩在 ~70-80% 阈值主动触发**，不等爆；保留决策、未决问题和偏好，丢冗余 tool 输出。
2. **压缩前先 memory-flush**（参考 OpenClaw）：插一个 silent 步骤让模型把没落盘的重要事实写进 MEMORY.md 再压。
3. **子 agent 卸载**：nuwa / darwin / 重搜索 / 重浏览类任务用隔离子 agent 处理，主对话只收结论，详细过程留在子任务内。
4. **note-taking 文件外存**：长任务把进度、决策和待办写进任务笔记，压缩或断线后从文件恢复。
5. **项目指令 altitude 审计**：只常驻真正决定行为的硬约束，低频规则按需加载。
6. **tool 结果 cache-ttl 剪枝**（可选）：超大结果保留头尾，过期后清理，控制上下文成本。

### 5.1 当前状态边界（2026-07）

FanBox 当前通过 PTY 承载用户选择的 CLI，真实上下文仍由各 CLI 自己管理。此前的无头会话桥、桌面聊天、自动 compact 和会话进度条已经随远程消息功能移除，因此本节的 memory-flush、子 agent 卸载和上下文可观测都只是候选路线，不是现有能力。
...
| **不可验证（文风/判断/产品决策）** | 改写作风格、调产品方向 | **不自动改**：agent 提议 → 用户确认才落库 | 人在回路 |
...
- 专门设计点：agent 在当前项目目录产出原型、文档或图片，主任务只接收成品路径和必要摘要，不回灌大段源码。
...
2. ❌ **后台定时 reflection / dreaming cron**——耗资源易跑偏，改为手动触发。
3. ❌ **对不可验证内容（文风/判断）的自动自改**——违反 verifiability constraint，只提议。
4. ❌ **消息平台网关**——偏离本地桌面驾驶舱的产品边界，也引入长期连接和账号维护成本。
