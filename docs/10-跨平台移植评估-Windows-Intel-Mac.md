...
- ffmpeg 探测同时覆盖 `/opt/homebrew` 与 `/usr/local`，兼容两种 Homebrew 默认前缀。
...
Claude Code 与 Codex 在 Windows 的安装、凭据和续会话行为由上游决定。应先用目标版本做最小真机探针；如果 CLI 自身不稳定，FanBox 只能如实降级，不能在应用层掩盖。
...
1. 先验证目标版本的 Claude Code、Codex 与 node-pty/ConPTY 基础链路。
