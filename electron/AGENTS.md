...
main.js: Electron 主进程编排入口，装配窗口、菜单、Codex 新会话快捷键、Shell/恢复/开发刷新领域服务、IPC 与应用生命周期，并仅在真实前台任务存在时确认退出
preload.js: contextBridge 安全桥接层，向渲染进程暴露终端、Codex 继续/新建启动、恢复、文件、剪贴板、拖拽、截图、更新与窗口事件的受控接口
