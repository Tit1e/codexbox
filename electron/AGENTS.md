...
pty-service.js: PTY 领域服务，管理终端与隐藏服务会话创建、顶层命令追踪、Ctrl+C 后等待 Shell 空闲的安全重启、当前目录、前台进程检测、运行任务快照与销毁
...
terminal-recovery-store.js: 终端恢复 JSON 仓储，保留隐藏服务身份并提供安全校验、去重、目录检查、一次性取出与清空
